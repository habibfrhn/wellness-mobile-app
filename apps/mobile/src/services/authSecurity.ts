...
const RATE_LIMIT_MARKERS = ["rate limit", "over_email_send_rate_limit", "over_request_rate_limit", "retry after"];
...
export function isRateLimitedAuthError(error: AuthLikeError | null | undefined) {
  if (!error) {
    return false;
  }
  ...
  return isRateLimitedError(error.message);
}
