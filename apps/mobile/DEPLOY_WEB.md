...
- Verify audio playback and analytics event ingestion. For soundscapes, run a timer past one natural file loop and confirm the browser does not issue duplicate full-file requests for the same selected asset.
...
- Keep web soundscapes on one looping `HTMLAudioElement` with metadata-first loading; avoid hidden duplicate preload elements unless a documented product requirement justifies the bandwidth cost.
