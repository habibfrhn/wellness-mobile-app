...
- Admin analytics data is fetched via the guarded `admin_audio_usage_analytics` RPC instead of broad direct table access; unused legacy admin analytics RPCs/views and tailored-session event acceptance paths were removed to reduce exposed read/ingest surfaces, and audio usage writes are restricted to the edge-function-backed `audio_play_sessions` flow.
...
rg -n "tailored|Tailored|tailored_session|session_mode" apps/mobile supabase README.md SECURITY_AUDIT.md -g '!node_modules'
